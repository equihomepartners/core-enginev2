# coding: utf-8

"""
    EQU IHOME SIM ENGINE API

    API for running home equity investment fund simulations

    The version of the OpenAPI document: 0.1.0
    Contact: info@equihome.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from equihome_sim_sdk.models.market_price_metrics import MarketPriceMetrics

class TestMarketPriceMetrics(unittest.TestCase):
    """MarketPriceMetrics unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> MarketPriceMetrics:
        """Test MarketPriceMetrics
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `MarketPriceMetrics`
        """
        model = MarketPriceMetrics()
        if include_optional:
            return MarketPriceMetrics(
                volatility = equihome_sim_sdk.models.volatility.Volatility(),
                alpha_idiosyncratic_share = 1.337,
                beta = equihome_sim_sdk.models.beta.Beta(),
                var = equihome_sim_sdk.models.var.Var(),
                cvar = equihome_sim_sdk.models.cvar.Cvar()
            )
        else:
            return MarketPriceMetrics(
                volatility = equihome_sim_sdk.models.volatility.Volatility(),
                beta = equihome_sim_sdk.models.beta.Beta(),
                var = equihome_sim_sdk.models.var.Var(),
                cvar = equihome_sim_sdk.models.cvar.Cvar(),
        )
        """

    def testMarketPriceMetrics(self):
        """Test MarketPriceMetrics"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
