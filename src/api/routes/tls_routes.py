"""
TLS API routes for the EQU IHOME SIM ENGINE v2.
...
@router.get("/suburbs/{suburb_id}/properties", response_model=List[PropertySummary])
async def get_suburb_properties(
    suburb_id: str,
    ...
    sort_by: Optional[str] = "base_value",
    sort_order: Optional[str] = "desc",
    limit: Optional[int] = 100,
    offset: Optional[int] = 0,
    tls_manager: Any = Depends(get_tls_manager_from_context),
    ...
    Args:
        ...
        sort_by: Field to sort by (base_value, bedrooms, bathrooms, property_type)
        sort_order: Sort order (asc, desc)
    ...
    try:
        ...
        # Get properties
        properties = suburb.get_properties_by_criteria(
        ...
        # Sort properties
        if sort_by not in ["base_value", "bedrooms", "bathrooms", "property_type"]:
            sort_by = "base_value"

        reverse = sort_order.lower() == "desc"

        if sort_by == "property_type":
            properties.sort(key=lambda p: p.property_type, reverse=reverse)
        else:
            properties.sort(key=lambda p: getattr(p, sort_by), reverse=reverse)
        ...
        logger.error("Failed to get suburb properties", error=str(e))
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/suburbs/{suburb_id}/properties/{property_id}", response_model=PropertyDetail)
async def get_property(
    ...
    try:
        ...
        # Get property
        prop = suburb.get_property(property_id)
        if not prop:
            raise HTTPException(
                status_code=404,
                detail=f"Property {property_id} not found in suburb {suburb_id}",
            )

        # Format metrics
        metrics = {}
        for metric_name, metric_value in prop.metrics.items():
            metrics[metric_name] = {
                "value": metric_value.value,
                "confidence": metric_value.confidence,
                "percentile": metric_value.percentile,
        ...
        return {
            ...
            "street_quality": prop.street_quality,
            "view_quality": prop.view_quality,
            "noise_level": prop.noise_level,
            "appreciation_modifier": prop.appreciation_modifier,
            "risk_modifier": prop.risk_modifier,
            "base_value": prop.base_value,
            "metrics": metrics,
        }

    except Exception as e:
        logger.error("Failed to get property", error=str(e))
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_properties(
    ...
    """
    Get properties across all suburbs.
    ...
    try:
        ...
        # Get properties
        property_tuples = tls_manager.get_properties_by_criteria(
            min_bedrooms=min_bedrooms,
            max_bedrooms=max_bedrooms,
            min_bathrooms=min_bathrooms,
            max_bathrooms=max_bathrooms,
        ...
        # Format properties
        result = []
        for suburb, prop in property_tuples:
            result.append({
                "property_id": prop.property_id,
                "suburb_id": prop.suburb_id,
                "property_type": prop.property_type,
...
async def get_metric_correlations(
    ...
    try:
        ...
        if not metrics:
            # Use top 20 metrics
            all_metrics = list(tls_manager.metrics.keys())
            metrics = all_metrics[:min(20, len(all_metrics))]
        else:
            # Validate metrics
            for metric in metrics:
                if metric not in tls_manager.metrics:
                    raise HTTPException(
                        status_code=404,
        ...
        for metric1 in metrics:
            ...
            for metric2 in metrics:
                correlation = tls_manager.get_metric_correlation(metric1, metric2)
                row.append(float(correlation))
            matrix.append(row)
...
async def get_property_distribution(
    ...
    try:
        ...
        for suburb in tls_manager.suburbs.values():
            for prop in suburb.properties.values():
